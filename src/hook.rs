...
use crate::{
    ...
    protocol::{
        ClientKind, Diagnostic, DiagnosticSeverity, EditKind, EditTarget,
        IDE_HOOK_CONTEXT_MAX_BYTES, IdeEditorContext, RpcRequest, RpcResponse,
    },
    workspace::Workspace,
};

const ABSOLUTE_HOOK_INPUT_MAX_BYTES: usize = 16 * 1024 * 1024;
const DIAGNOSTIC_HOOK_CONTEXT_MAX_BYTES: usize = 8 * 1024;
...
pub async fn run(command: HookCommand) -> anyhow::Result<()> {
    ...
    let context = match ClientContext::open(&input.cwd) {
        Ok(context) => context,
        Err(_) => return Ok(()),
    };
    if input_bytes > context.config.limits.max_hook_input_bytes {
        return Ok(());
    }
    let connector = match context.connector(ClientKind::Hook) {
...
fn format_diagnostics(diagnostics: &[Diagnostic]) -> String {
    ...
    for diagnostic in diagnostics
        ...
        .take(edit_diagnostics::HOOK_MAX_ERRORS)
    {
    ...
    if output.len() > DIAGNOSTIC_HOOK_CONTEXT_MAX_BYTES {
        let mut end = DIAGNOSTIC_HOOK_CONTEXT_MAX_BYTES;
