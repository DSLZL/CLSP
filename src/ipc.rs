...
impl ClientContext {
    pub(crate) fn open(path: &Path) -> Result<Self, ClspError> {
    ...
    pub(crate) fn connector(&self, client_kind: ClientKind) -> Result<BrokerConnector, ClspError> {
        let paths = StatePaths::for_workspace(&self.workspace.hash())?;
        Ok(BrokerConnector::new(
            &self.workspace,
            &paths,
...
impl BrokerConnector {
    pub fn for_workspace(path: &Path, client_kind: ClientKind) -> Result<Self, ClspError> {
        ClientContext::open(path)?.connector(client_kind)
    }
